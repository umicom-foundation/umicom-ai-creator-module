...
int main(void)
{
    ...
    const UmiExperienceLayoutDefinition *layout =
        umi_ai_creator_workspace_default();
    const UmiExperienceFeatureDefinition *next =
        umi_ai_creator_workspace_next_feature();

    if (definition == NULL ||
        umi_ai_creator_application_status(&status) != UMI_STATUS_OK ||
        layout == NULL) {
        fputs("Umicom AI Creator: Framework experience unavailable\n", stderr);
        return 1;
    }
